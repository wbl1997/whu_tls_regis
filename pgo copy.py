...
def pgo(relative_pose_file, absolute_pose_file, loop_index_list = [], output_file="optimized_poses.txt"):
    ...
    for i, (timestamp, pose) in enumerate(relative_poses):
        ...
        key_point.append(gtsam.symbol('p', i))
        initial_estimate.insert(key_point[i], interpolated_absolute_poses[i][1].translation())
    ...
    #         odomNoiseVector6 = np.array([1e-6, 1e-6, 1e-6, 1e-3, 1e-3, 1e-3])
    #         noise_model = gtsam.noiseModel.Diagonal.Variances(odomNoiseVector6)
    #         graph.add(gtsam.PriorFactorPose3(key[i], interpolated_absolute_poses[i][1], noise_model))

    # # 添加重叠位姿约束
    # for i in range(len(relative_poses)-1):
    #     if overlap_mask[i] == 1 and overlap_mask[i+1] == 1:
    #         odomNoiseVector6 = np.array([1e-120, 1e-120, 1e-120, 1e-120, 1e-120, 1e-120])
    #         # odomNoiseVector6 = np.array([1e-6, 1e-6, 1e-6, 1e-3, 1e-3, 1e-3])
    #         noise_model = gtsam.noiseModel.Diagonal.Variances(odomNoiseVector6)
    #         graph.add(gtsam.BetweenFactorPose3(key[i], key[i+1], interpolated_absolute_poses[i][1].between(interpolated_absolute_poses[i+1][1]), noise_model))
            
    # # 添加闭环约束
    # T_lc = gtsam.Pose3()
    # odomNoiseVector6 = np.array([1e-6, 1e-6, 1e-6, 1e-3, 1e-3, 1e-3])
    # odomNoiseVector6 = 0.5 * odomNoiseVector6
    # noise_model = gtsam.noiseModel.Diagonal.Variances(odomNoiseVector6)
    # for loop_index in loop_index_list:
    #     T_lc = interpolated_absolute_poses[loop_index[0]][1].between(interpolated_absolute_poses[loop_index[1]][1])
    #     graph.add(gtsam.BetweenFactorPose3(key[loop_index[0]], key[loop_index[1]], T_lc, noise_model))

    # 添加闭环约束（仅位置约束）
    odomNoiseVector3 = np.array([1e-6, 1e-6, 1e-6])
    noise_model = gtsam.noiseModel.Diagonal.Variances(odomNoiseVector3)
    for loop_index in loop_index_list:
        ...
        graph.add(gtsam.BetweenFactorPoint3(key_point[loop_index[0]], key_point[loop_index[1]], pos_lc, noise_model))
