...
def pgo(relative_pose_file, absolute_pose_file, loop_index_list = [], output_file="optimized_poses.txt"):
    ...
    # 计算relative_pose 和 absolute_pose的相对世界坐标系变换，根据第一帧的绝对位姿
    # Tw1w2 = relative_poses[0][1].between(interpolated_absolute_poses[0][1])
    Tw1w2 = interpolated_absolute_poses[0][1].between(relative_poses[0][1])
    relative_poses = [(timestamp, Tw1w2.between(pose)) for timestamp, pose in relative_poses]
    print("relative_poses[0]: ", relative_poses[0])
    print("interpolated_absolute_poses[0]: ", interpolated_absolute_poses[0])
    ...
    for i, (timestamp, pose) in enumerate(relative_poses):
        ...
        initial_estimate.insert(key[i], relative_poses[i][1])
    ...
    # 添加全局位姿的绝对观测
    absolute_pose_noise = gtsam.noiseModel.Diagonal.Variances(np.array([1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3]))
    for i in range(len(relative_poses)):
        if overlap_mask[i] == 1:
            graph.add(gtsam.PriorFactorPose3(key[i], interpolated_absolute_poses[i][1], absolute_pose_noise))
